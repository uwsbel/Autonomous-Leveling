...
//
// =============================================================================
// Author: Harry Zhang, Ganesh Avrivoli, Huzaifa Unjhawala, Radu Serban
// =============================================================================
//
...
bool GetProblemSpecs(int argc,
    ...
    // Add options for pile_height only with its default value
    ...
    // Retrieve pile_height value from CLI
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "CHRONO_DATA_DIR: " << CHRONO_DATA_DIR << std::endl;
    ...
    // std::string blade_pv_filename = "../data/control_commands/" + std::to_string(pile_max_height) + "_" + push_seq + ".txt";
    ...
    const std::string out_dir = "../data/output/" + std::to_string(pile_max_height) + "/soil_leveling_" + push_seq;
    ...
    // save particle frames data
    // Create CSV file for logging vehicle data
    const std::string pc_dir = "../data/output/" + std::to_string(pile_max_height) + "/soil_leveling_" + push_seq + "/pc_frames";
    if (!std::filesystem::exists(pc_dir)) {
        std::filesystem::create_directories(pc_dir);
    }
    ...
    while (time < tend && !simulation_complete) {
        ...
        switch (vehicle_state) {
            ...
            case VehicleState::WAIT_AT_NEGATIVE: {
                ...
                if (time >= t_back + 1.0) {
                    ...
                    std::string new_control_command_file = "../data/control_commands/" + std::to_string(pile_max_height) + "_" + push_seq + ".txt";
        ...
        csv_file << time << "," 
                 ...
                 << blade_rot_angles.x() << "," << blade_rot_angles.y() << "," << blade_rot_angles.z() << "\n";
        csv_file.flush();
